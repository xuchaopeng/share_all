...
- **事件总线**
  ...
  - **注意**
    ...
    - 3、其实我们开发过程中,尤其是组件化开发（当然不局限于vue）过程中，`事件池`带给我们的是一种处理问题的思维模式。例如，微信小程序开发 onReachBottomDistance（页面上拉触底事件），项目中多个地方依赖这个触底事件，可考虑用`事件池`。
...
## 混入
 ...
 - **合并问题**
  ...
  - 2、当 `组件内` 与 `mixin` 定义相同的属性或方法（非钩子函数）。`组件内`权重最高，`mixin`中定义的属性方法会被盖掉。
...
elementUI 表单提交组件简版的实现，是对已上内容的综合实践。一个很典型的组件实现的例子。(打开你高阶组件封装的思维)
